...
const XSD_STRING = N3.DataFactory.namedNode(
...
export async function executeAllConstructs(
  ...
  for (const source of sources) {
    ...
    try {
      ...
      console.debug(
        `Source "${source.name}": instance store has ${instanceStore.size} quads, prefix="${sourcePrefix}"`,
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Unknown error';
      warnings.push(
        `Source "${source.name}": failed to parse instances — ${msg}`,
      );
    ...
    for (const sparqlQuery of constructQueries) {
      try {
        ...
        const msg = err instanceof Error ? err.message : 'Unknown error';
        warnings.push(
          `Source "${source.name}": CONSTRUCT query failed — ${msg}`,
        );
        continue;
      }
    }
