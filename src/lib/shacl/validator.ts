...
export async function validateWithShacl(
  ...
  try {
    ...
    const validator = new SHACLValidator(shapesStore as any, { factory: ShaclFactory as any })
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    return report.results.map((result) => {
      ...
      let targetClassUri = ''
      if (sourceShapeTerm) {
        const parentShapes = shapesStore.getSubjects(SH_PROPERTY, sourceShapeTerm, null)
      ...
      const message =
        messages && messages.length > 0 ? (messages[0]!.value ?? 'SHACL violation') : 'SHACL violation'
