...
export function xmlToInstances(xml: string, schemaNodes: SourceNodeData[]): N3.Store {
  ...
  function walkEl(el: Element, depth: number): N3.BlankNode | undefined {
    ...
    store.addQuad(quad(instance, namedNode(RDF_TYPE), namedNode(uriBase + toPascalCase(el.localName)), defaultGraph()))

    for (let i = 0; i < el.attributes.length; i++) {
      const attr = el.attributes[i]!
      store.addQuad(quad(instance, namedNode(uriBase + 'attr_' + attr.localName), typedLiteral(attr.value), defaultGraph()))
    }
    ...
    for (let i = 0; i < el.childNodes.length; i++) {
      ...
      if (hasChildElements(childEl)) {
        ...
        if (childBN) {
          store.addQuad(quad(instance, namedNode(uriBase + childEl.localName), childBN, defaultGraph()))
        }
      } else {
        const text = childEl.textContent ?? ''
        store.addQuad(quad(instance, namedNode(uriBase + childEl.localName), typedLiteral(text), defaultGraph()))
      }
    }
    return instance
  }
...
export function sourceToInstances(source: { rawData: string; dataFormat: 'json' | 'xml'; schemaNodes: SourceNodeData[] }): N3.Store {
  if (source.dataFormat === 'xml') return xmlToInstances(source.rawData, source.schemaNodes)
  return jsonToInstances(source.rawData, source.schemaNodes)
}

export function jsonToInstances(json: string, schemaNodes: SourceNodeData[]): N3.Store {
  const store = new N3.Store()
  const uriBase = schemaNodes[0]?.data.prefix ?? ''
