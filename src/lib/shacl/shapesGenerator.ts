...
export function generateShapes(ontologyNodes: OntologyNode[]): N3.Store {
  ...
  for (const node of ontologyNodes) {
    ...
    store.addQuad(quad(shapeNode, namedNode(RDF_TYPE), namedNode(SH + 'NodeShape'), defaultGraph()))
    // shapeUri sh:targetClass classUri
    store.addQuad(quad(shapeNode, namedNode(SH + 'targetClass'), classNode, defaultGraph()))
    ...
    for (const prop of node.data.properties) {
      ...
      if (prop.kind === 'datatype') {
        ...
        store.addQuad(quad(bn, namedNode(SH + 'path'), propNode, defaultGraph()))
        store.addQuad(quad(bn, namedNode(SH + 'datatype'), namedNode(datatypeUri), defaultGraph()))
        store.addQuad(quad(shapeNode, namedNode(SH + 'property'), bn, defaultGraph()))
      } else if (prop.kind === 'object') {
        const rangeNode = namedNode(prop.range)

        store.addQuad(quad(bn, namedNode(SH + 'path'), propNode, defaultGraph()))
        store.addQuad(quad(bn, namedNode(SH + 'class'), rangeNode, defaultGraph()))
        store.addQuad(quad(shapeNode, namedNode(SH + 'property'), bn, defaultGraph()))
      }
    }
  }
