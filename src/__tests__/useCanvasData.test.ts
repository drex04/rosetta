...
describe('useCanvasData', () => {
  ...
  it('case 4: mapping edges use "mapping_<uuid>" id — stripping prefix yields the raw mapping id', () => {
    const masterNode = { id: 'master-n1', type: 'classNode' as const, position: { x: 0, y: 0 }, data: { label: 'Master', uri: 'http://onto.test/Class', prefix: '', properties: [] } } as OntologyNode
    useOntologyStore.setState({ nodes: [masterNode], edges: [], turtleSource: '' })

    const sourceNode: Node = { id: 'src-n1', position: { x: 100, y: 100 }, data: { label: 'Source', uri: 'http://src.test/Class', prefix: '', properties: [] } }
    const source: Source = {
      id: 'src-1', name: 'S', order: 0, rawData: '{}', dataFormat: 'json' as const,
      schemaNodes: [sourceNode], schemaEdges: [], turtleSource: '', parseError: null,
    }
    useSourcesStore.setState({ sources: [source], activeSourceId: 'src-1' })
    ...
    const { result } = renderHook(() => useCanvasData())
    const mappingEdge = result.current.edges.find((e) => e.id.startsWith('mapping_'))
