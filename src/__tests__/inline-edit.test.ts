...
describe('updateNode', () => {
  it('updates label and URI correctly on the target node', () => {
    ...
    useOntologyStore.getState().updateNode('test-1', { label: 'NewLabel', uri: 'ex:NewClass' })

    const node = useOntologyStore.getState().nodes.find((n) => n.id === 'test-1')!
...
describe('updateProperty', () => {
  it('updates property label only and does NOT fire onInvalidateMappings', () => {
    ...
    useOntologyStore.getState().addNode(makeNode('test-1', [makeProperty('ex:name', 'name')]))

    useOntologyStore.getState().updateProperty('test-1', 'ex:name', { label: 'fullName' })
  ...
  it('fires onInvalidateMappings with the old URI when property URI changes', () => {
    ...
    useOntologyStore.getState().updateProperty('test-1', 'ex:name', { uri: 'ex:fullName' })
...
describe('updateSchemaNode', () => {
  it('updates the correct node in the correct source', () => {
    ...
    useSourcesStore.getState().updateSchemaNode('src-1', 'sn-1', { label: 'Updated', uri: 'ex:Updated' })

    const src = useSourcesStore.getState().sources.find((s) => s.id === 'src-1')!
  ...
  it('is a safe no-op when nodeId does not exist in the source', () => {
    ...
    useSourcesStore.getState().updateSchemaNode('src-1', 'nonexistent', { label: 'Ghost' })
...
describe('commitProp validation logic', () => {
  interface Prop { uri: string; label: string }
  ...
  function validateProp(
    ...
    if (siblings.some((p) => p.uri !== editingPropUri && p.label === draftLabel.trim())) {
  ...
  it('duplicate property name among siblings produces an error', () => {
    ...
    expect(validateProp('speed', 'ex:name', siblings)).toBe('Duplicate property name')
  })
