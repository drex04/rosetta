...
function addMapping(overrides: Partial<Parameters<typeof useMappingStore.getState['prototype']['addMapping']>[0]> = {}) {
...
describe('MappingGroup store', () => {
  ...
  it('createGroup sets groupId on member mappings and returns a group ID', () => {
    ...
    const groupId = useMappingStore.getState().createGroup('src1', [id1, id2], 'concat')
  ...
  it('createGroup sparqlConstruct is populated (not empty) after creation', () => {
    const id1 = addMapping({ sourcePropUri: 'http://example.org/ns#firstName' })
    const id2 = addMapping({ sourcePropUri: 'http://example.org/ns#lastName' })
    const groupId = useMappingStore.getState().createGroup('src1', [id1, id2], 'concat')
    const group = useMappingStore.getState().getGroupsForSource('src1').find((g) => g.id === groupId)
  ...
  it('allows grouping mappings with same targetClassUri+targetPropUri (duplicate target detection)', () => {
    ...
    const id1 = addMapping({ sourcePropUri: 'ex:first', targetPropUri: 'nato:name' })
    const id2 = addMapping({ sourcePropUri: 'ex:last', targetPropUri: 'nato:name' })
...
describe('generateGroupConstruct', () => {
  it('concat with 2 members produces SPARQL containing CONCAT, BIND, and STR', () => {
    ...
    const m1 = makeMapping({ sourcePropUri: 'http://example.org/ns#firstName', groupOrder: 0 })
    const m2 = makeMapping({ sourcePropUri: 'http://example.org/ns#lastName', groupOrder: 1 })
  ...
  it('coalesce uses OPTIONAL and COALESCE', () => {
    ...
    const m1 = makeMapping({ sourcePropUri: 'http://example.org/ns#primaryName', groupOrder: 0 })
    const m2 = makeMapping({ sourcePropUri: 'http://example.org/ns#alternateName', groupOrder: 1 })
    const sparql = generateGroupConstruct(group, [m1, m2])
  ...
  it('template substitution: "{0}, {1}" with 2 members produces correct CONCAT expression', () => {
    ...
    const m1 = makeMapping({ sourcePropUri: 'http://example.org/ns#city', groupOrder: 0 })
    const m2 = makeMapping({ sourcePropUri: 'http://example.org/ns#country', groupOrder: 1 })
    const sparql = generateGroupConstruct(group, [m1, m2])
  ...
  it('members are sorted by groupOrder so v0 corresponds to lowest groupOrder', () => {
    ...
    const mHigh = makeMapping({ sourcePropUri: 'http://example.org/ns#second', groupOrder: 1 })
    const mLow  = makeMapping({ sourcePropUri: 'http://example.org/ns#first',  groupOrder: 0 })
