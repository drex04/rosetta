...
const makeSubclassEdge = (id: string, source: string, target: string): OntologyEdge =>
  ({
    id,
    source,
    target,
...
const makeObjectPropertyEdge = (id: string, source: string, target: string): OntologyEdge =>
  ({
    id,
    source,
    target,
...
describe('ontologyStore.replaceEdge', () => {
  ...
  it('no-op when oldId not found — edge list unchanged', () => {
    ...
    useOntologyStore.getState().replaceEdge('nonexistent', makeSubclassEdge('e2', 'X', 'Y'))
  ...
  it('replaced edge has new type and data', () => {
    ...
    expect((edges[0].data as { predicate: string }).predicate).toBe('owl:ObjectProperty')
  })
})
...
describe('handleEdgePickerSelect — mode dispatch (via store)', () => {
  it('create-onto: addEdge adds correct subclassEdge type', () => {
    ...
    expect((edges[0].data as { predicate: string }).predicate).toBe('rdfs:subClassOf')
  })
  ...
  it('create-onto: addEdge adds correct objectPropertyEdge type', () => {
    ...
    expect(edges).toHaveLength(1)
    expect(edges[0].type).toBe('objectPropertyEdge')
    expect((edges[0].data as { uri: string }).uri).toBe('http://example.org/e-obj')
  })
  ...
  it('edit mode: replaceEdge with unknown id is a no-op (original edge preserved)', () => {
    ...
    useOntologyStore.getState().replaceEdge('does-not-exist', makeObjectPropertyEdge('e-new', 'X', 'Y'))
  ...
  it('edit mode (source edge): updateSource replaces edge in schemaEdges', () => {
    ...
    const updated = useSourcesStore.getState().sources.find((s) => s.id === 'src-1')
