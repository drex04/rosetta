  ...
  function commitProp(): boolean {
    ...
    if (
      data.properties.some(
        (p) => p.uri !== editingPropUri && p.label === draftPropLabel.trim(),
      )
    ) {
      ...
      >
        ...
        {editingHeader ? (
          ...
          >
            <input
              autoFocus
              className="text-white bg-master/80 border border-white/40 rounded px-1 py-0.5 text-sm font-medium w-full"
              value={draftLabel}
