...
export const useMappingStore = create<MappingStateInternal>((set, get) => ({
  ...
  removeMapping: (id) => {
    set((s) => {
      ...
      return {
        mappings: updated,
        selectedMappingId:
          s.selectedMappingId === id ? null : s.selectedMappingId,
      };
    });
  },
  ...
  createGroup: (sourceId, mappingIds, strategy) => {
    ...
    const members = mappingIds
      .map((id) => allMappings.find((m) => m.id === id))
      .filter(Boolean) as Mapping[];
    ...
    set((s) => {
      ...
      for (const [sid, list] of Object.entries(s.groups)) {
        updatedGroups[sid] = list.map((g) => {
          ...
          const strategy =
            (patch as { strategy?: MappingGroup['strategy'] }).strategy ??
            g.strategy;
          ...
          if (strategy === 'template') {
            updated = {
              ...
              templatePattern:
                (merged as { templatePattern?: string }).templatePattern ?? '',
            };
          } else if (strategy === 'coalesce') {
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            const { templatePattern: _tp, ...rest } = merged as MappingGroup & {
              templatePattern?: string;
            };
