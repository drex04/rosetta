...
export function useCanvasData(): {
  nodes: (OntologyNode | SourceNodeData)[];
  edges: (OntologyEdge | MappingFlowEdge)[];
} {
  ...
  const mappingEdgesBase = useMemo((): MappingFlowEdge[] => {
    ...
    const activeMappings = activeSourceId
      ? (mappings[activeSourceId] ?? [])
      : [];

    for (const mapping of activeMappings) {
      // Scope source node lookup to the correct source (by sourceId) to handle duplicate URIs
      const sourceWithMapping = sources.find(
        (src) => src.id === mapping.sourceId,
      );
      const sourceNode = sourceWithMapping?.schemaNodes.find(
        (n) => n.data.uri === mapping.sourceClassUri,
      );
      // Find the master ReactFlow node whose data.uri matches
      const targetNode = masterNodes.find(
  ...
  const mappingEdges = useMemo(
    () =>
      mappingEdgesBase.map((e) => ({
        ...e,
        selected:
          (e.data as { mappingId?: string })?.mappingId === selectedMappingId,
        data: {
          ...(e.data as object),
          selected:
            (e.data as { mappingId?: string })?.mappingId === selectedMappingId,
