...
test.describe('Canvas ↔ editor sync', () => {
  test('42 - Editor change updates canvas after debounce', async ({ freshPage: page }) => {
  ...
  test('43 - Pending edits dialog on canvas change while editing', async ({ freshPage: page }) => {
    await loadExampleProject(page)
    await clickTab(page, 'Ontology tab')
    ...
    const secondPill = page.getByRole('button', { name: /Select source/ }).nth(1)
