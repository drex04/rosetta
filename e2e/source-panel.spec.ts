...
test.describe('Source panel', () => {
  test('11 - Valid JSON populates Turtle preview', async ({
    freshPage: page,
  }) => {
  ...
  test('13 - Edit source name in SRC panel updates pill', async ({
    freshPage: page,
  }) => {
    await page.getByRole('button', { name: 'Add new source' }).click();
    await page
    ...
    await expect(
      page.getByRole('button', { name: 'Select source Alpha' }),
    ).toBeVisible();
  });
});
