...
test.describe('Canvas', () => {
  test('29 - Class nodes appear after example project', async ({ freshPage: page }) => {
  ...
  test('30 - Source nodes appear when source selected', async ({ freshPage: page }) => {
    await loadExampleProject(page)
  ...
  test('34 - Source nodes switch with active source', async ({ freshPage: page }) => {
    await loadExampleProject(page)
    ...
    const differentNodes = idsA.some((id) => !idsB.includes(id)) || idsB.some((id) => !idsA.includes(id))
