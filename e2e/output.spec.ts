...
test.describe('Output (OUT tab)', () => {
  test('21 - Turtle output is non-empty after example project', async ({ freshPage: page }) => {
  ...
  test('22 - JSON-LD output is non-empty after example project', async ({ freshPage: page }) => {
    await loadExampleProject(page)
    await clickTab(page, 'Output tab')
    ...
    await page.locator('[aria-label="JSON-LD output"]').waitFor({ state: 'visible', timeout: 15000 })

    const content = await page.locator('[aria-label="JSON-LD output"]').locator('.cm-content').textContent()
