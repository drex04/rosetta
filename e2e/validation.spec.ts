...
test.describe('Validation panel', () => {
  test('VAL-1 - VAL tab is present in right panel', async ({
    freshPage: page,
  }) => {
  ...
  test('VAL-2 - VAL tab shows "Click Validate to run" before first run', async ({
    freshPage: page,
  }) => {
  ...
  test('VAL-4 - After validate with no mappings, VAL tab shows "All valid"', async ({
    freshPage: page,
  }) => {
    await page.getByRole('button', { name: 'Add new source' }).click();
    await page
  ...
  test('VAL-6 - Source pills show ○ before validation, ✓ after valid run', async ({
    freshPage: page,
  }) => {
    ...
    const pillContainer = page
      .locator('[aria-label="Source selector"] > div')
      .first();
    ...
    test.skip(
      true,
      'The ✓ assertion requires Validate button + full runValidation implementation',
    );
  });

  test('VAL-7 - Clicking a violation with canvasNodeId applies ring to SourceNode', async ({
    freshPage: page,
  }) => {
    await loadExampleProject(page);
    await page.getByRole('button', { name: 'Validate' }).click();
    ...
    if (canvasNodeId) {
      const nodeWrapper = page
        .locator(`[data-id="${canvasNodeId}"] > div`)
        .first();
