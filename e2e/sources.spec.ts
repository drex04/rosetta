...
test.describe('Source management', () => {
  ...
  test('5 - Rename source via Enter', async ({ freshPage: page }) => {
    ...
    await page.getByRole('button', { name: 'Select source Source 1' }).dblclick()
    ...
    await expect(page.getByRole('button', { name: 'Select source MySource' })).toBeVisible()
  })
  ...
  test('6 - Rename cancel via Escape', async ({ freshPage: page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Select source Source 1' })).toBeVisible()
  })
  ...
  test('7 - Delete source with no data', async ({ freshPage: page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Select source Source 1' })).toHaveCount(0)
  })
  ...
  test('10 - Switch between sources changes SRC content', async ({ freshPage: page }) => {
    ...
    await page.getByRole('button', { name: 'Add new source' }).click()
    await page.getByRole('button', { name: 'Select source Source 2' }).waitFor({ state: 'visible' })
    ...
    const editorContent1 = await page.locator('[aria-label="JSON source editor"] .cm-content').textContent()
    ...
    await page.waitForTimeout(300)
    const editorContent2 = await page.locator('[aria-label="JSON source editor"] .cm-content').textContent()
