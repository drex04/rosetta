...
test.describe('Persistence', () => {
  test('25 - State survives page reload', async ({ freshPage: page }) => {
    ...
    const sourcePillCount = await page
  ...
  test('26 - New project clears IDB', async ({ freshPage: page }) => {
    ...
    await expect(
      page.getByRole('button', { name: /Select source/ }),
    ).toHaveCount(0);
  });

  test('27 - Active tab persists across reload', async ({
    freshPage: page,
  }) => {
    await loadExampleProject(page);
  ...
  test('28 - Save status shows Saving then Saved', async ({
    freshPage: page,
  }) => {
