...
test.describe('Integration', () => {
  test('44 - Complete mapping workflow', async ({ freshPage: page }) => {
    ...
    const edgeCountBefore = await page
    ...
    const sourceHandle = page
      .locator('.react-flow__node-sourceNode [data-handleid^="prop_"]')
      .first();
    const targetHandle = page
      .locator('.react-flow__node-classNode [data-handleid^="target_prop_"]')
      .first();
    ...
    if (sourceBox && targetBox) {
      ...
      const edgeCountAfter = await page
        .locator('.react-flow__edge[data-id^="mapping_"]')
        .count();
  ...
  test('45 - Example project full round-trip', async ({ freshPage: page }) => {
    ...
    const restoredSourceCount = await page
      .getByRole('button', { name: /Select source/ })
      .count();
