...
test.describe('Edge cases', () => {
  test('38 - Delete source with mappings removes orphan edges', async ({
    freshPage: page,
  }) => {
    await loadExampleProject(page);
    const mappingEdgesBefore = await page
    ...
    await page
      .getByRole('button', { name: `Delete source ${sourceName}` })
      .click();
    ...
    const mappingEdgesAfter = await page
      .locator('.react-flow__edge[data-id^="mapping_"]')
      .count();
    expect(mappingEdgesAfter).toBeLessThan(mappingEdgesBefore);
  });

  test('39 - Rapid tab switching does not crash', async ({
    freshPage: page,
  }) => {
    await loadExampleProject(page);
  ...
  test('40 - Clear JSON then re-paste', async ({ freshPage: page }) => {
    ...
    await expect(
      page.locator('[aria-label="Generated Turtle preview"]'),
    ).toBeVisible();
